...
const LiveCountdown = ({ timestamp, onExpire }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (diff <= 0) { clearInterval(timer); setTimeLeft("00h 00m 00s"); onExpire && onExpire(); }
      else { const h = Math.floor(diff / 3600000); const m = Math.floor((diff % 3600000) / 60000); const s = Math.floor((diff % 60000) / 1000); setTimeLeft(`${h}h ${m < 10 ? '0' + m : m}m ${s < 10 ? '0' + s : s}s`); }
    }, 1000);
...
const App = () => {
  ...
  useEffect(() => {
    onSnapshot(collection(db, "liveMocks"), (s) => setLiveMocks(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a,b) => (b.timestamp || 0) - (a.timestamp || 0))));
    onSnapshot(collection(db, "practiceSets"), (s) => setPracticeSets(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a,b) => (b.timestamp || 0) - (a.timestamp || 0))));
  ...
  const handleStartExamFlow = (exam) => {
    const h = parseInt(exam.hours) || 0; const m = parseInt(exam.minutes) || 0;
    setCurrentExam({ ...exam, duration: (h * 3600) + (m * 60) || 3600 });
    setShowNameModal(true); setStudentNameInput(''); setStudentCodeInput('');
  };
  ...
  const finalizeExamStart = () => {
    ...
    setIsExamActive(true); setShowNameModal(false);
  };
  ...
  return (
    <div className="min-h-screen font-sans text-white select-none flex flex-col items-center overflow-x-hidden transition-all duration-700 bg-black" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8)), url(${APP_BACKGROUND_URL})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} >
      ...
      <main className="w-full max-w-6xl p-4 mb-20">
        {activeTab === 'home' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in w-full">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-black/60 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border-2 border-white/10 text-center">
                <GraduationCap size={48} className="text-blue-400 mx-auto mb-3 animate-bounce-slow" />
                <h2 className="text-xl md:text-3xl font-black uppercase italic tracking-tight text-white">Elevate Your Mathematics <br/> <span className="text-blue-400 underline decoration-yellow-400 decoration-2 underline-offset-8">with Anshu Sir</span></h2>
                <div className="mt-8 p-6 bg-white/5 rounded-3xl border border-white/10 shadow-inner">
                  <p className="text-slate-400 font-bold uppercase italic text-[11px] mb-4 tracking-widest">Contact Anshu Sir on WhatsApp to become a Registered Student</p>
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-2xl border-4 border-blue-600 text-slate-800">
                ...
                <div className="overflow-x-auto p-2">
                  <table className="w-full text-[9px] text-left border-collapse font-bold uppercase italic">
                    <thead><tr className="border-b-2 border-slate-100 text-slate-400"><th className="p-3">Features</th><th className="p-3">Guest Student</th><th className="p-3 text-blue-600">Registered</th></tr></thead>
                    <tbody>
                      <tr className="border-b border-slate-50"><td className="p-3">Mock Access</td><td className="p-3 text-slate-400">Limited</td><td className="p-3 text-green-600">Unlimited</td></tr>
                      <tr className="border-b border-slate-50"><td className="p-3">Exam Mode</td><td className="p-3 text-slate-400">Only MCQ</td><td className="p-3 text-green-600">MCQ + Written</td></tr>
                      <tr className="border-b border-slate-50"><td className="p-3">Performance</td><td className="p-3 text-slate-400">No Transcript</td><td className="p-3 text-green-600">Full Transcript</td></tr>
                      <tr className="border-b border-slate-50"><td className="p-3">Live Mocks</td><td className="p-3 text-slate-400">No Entry</td><td className="p-3 text-green-600">Exclusive Access</td></tr>
                      <tr><td className="p-3">Features</td><td className="p-3 text-slate-400">Limited</td><td className="p-3 text-green-600">All Features</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </div>
            <div className="bg-black/60 backdrop-blur-xl p-5 rounded-3xl shadow-md border border-white/10 text-left h-fit">
              ...
              <div className="space-y-3">
                {activityLogs.slice(0, 10).map(log => (
                  <div key={log.id} className="p-2.5 bg-white/5 rounded-xl border-l-4 border-blue-600 shadow-sm transition-all hover:bg-white/10">
                    <p className="text-[10px] font-black uppercase text-white truncate">{log.studentName}</p>
                    <p className="text-[8px] font-bold text-slate-400 uppercase italic truncate">{log.examTitle}</p>
                  </div> ))}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'live' && (
          <div className="space-y-4 w-full text-left print:hidden">
            ...
            {ongoingLive.length > 0 ? ongoingLive.map((m, i) => (
              <div key={m.id} className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl shadow-xl flex justify-between items-center border border-white/10">
                <div className="flex-1 pr-4">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-black uppercase italic tracking-tighter text-white">{m.name}</h3>
                    {m.level && <span className={`text-[7px] px-1.5 py-0.5 rounded font-black italic ${m.level === 'HARD' ? 'bg-red-600' : m.level === 'MODERATE' ? 'bg-yellow-600' : 'bg-green-600'}`}>{m.level}</span>}
                  </div>
              ...
              </div> )) : <p className="text-[10px] text-slate-500 italic p-4 text-center">No active sessions.</p>}
          </div>
        )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {allMocks.filter(m => (m.class || 'Other') === cls).map((p, i) => (
                      <div key={p.id} className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl shadow flex justify-between items-center border border-white/10 hover:border-blue-500/50 transition-all">
                        <div className="flex-1 pr-4">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-bold uppercase text-xs italic text-white">{p.name}</h3>
                            {p.level && <span className={`text-[7px] px-1.5 py-0.5 rounded font-black italic ${p.level === 'HARD' ? 'bg-red-600' : p.level === 'MODERATE' ? 'bg-yellow-600' : 'bg-green-600'}`}>{p.level}</span>}
                          </div>
                          <p className="text-[9px] font-bold text-slate-500 uppercase italic">Time: {p.hours || 0}h {p.minutes || 0}m</p>
                        </div>
...
const TeacherZoneMainView = ({ liveMocks, practiceSets, students, teacherPin, setTeacherPin, studentResults }) => {
  ...
  const PaperManager = ({ title, items, type, color }) => (
    <div className="bg-black/60 backdrop-blur-xl rounded-[2rem] shadow-2xl border-t-8 border-slate-900 mb-8 w-full overflow-hidden border-x border-b border-white/5">
      <div className="flex justify-between items-center p-6 border-b border-white/5 text-left"> <h3 className={`font-black uppercase text-xs italic ${color}`}>{title} Manager ({items.length})</h3> </div>
      <div className="max-h-[500px] overflow-y-auto p-4 space-y-3 bg-white/5 no-scrollbar">
        {items.map((item, index) => (
          <div key={item.id} className="bg-slate-900/60 rounded-2xl border border-white/10 overflow-hidden">
            <div onClick={() => setExpandedId(expandedId === item.id ? null : item.id)} className="p-4 flex justify-between items-center cursor-pointer hover:bg-white/5 group text-left">
              <div className="flex-1 pr-2">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${item.isPublished ? 'bg-green-500 animate-pulse' : 'bg-slate-700'}`}></div>
                  <span className="text-xs font-black uppercase text-white truncate max-w-[150px]">{item.name}</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={async (e) => { e.stopPropagation(); if(window.confirm("Delete?")) await deleteDoc(doc(db, type === 'live' ? 'liveMocks' : 'practiceSets', item.id)); }} className="p-2 text-slate-600 hover:text-red-500 transition-colors"><Trash2 size={16}/></button>
            ...
            {expandedId === item.id && (
              <div className="p-5 border-t border-white/5 bg-black/40 space-y-4 animate-in slide-in-from-top-2 text-left">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div><p className="text-[8px] font-black text-yellow-400 uppercase mb-1">Level</p><select value={item.level || 'EASY'} onChange={(e) => updateField(item.id, type, 'level', e.target.value)} className="w-full p-2 bg-black border border-white/10 rounded-xl text-white text-xs font-black"><option value="EASY">EASY</option><option value="MODERATE">MODERATE</option><option value="HARD">HARD</option></select></div>
                  <div className="flex items-center gap-2 mt-4"><input type="checkbox" checked={item.isGuestEnabled} onChange={(e) => updateField(item.id, type, 'isGuestEnabled', e.target.checked)} className="accent-green-500 w-4 h-4" /><p className="text-[10px] font-black uppercase text-green-400 italic">Guest Mode</p></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Exam Name</p><input type="text" defaultValue={item.name} onBlur={(e) => updateField(item.id, type, 'name', e.target.value.toUpperCase())} className="w-full p-2.5 rounded-xl border border-white/10 bg-black text-white text-xs font-black outline-none focus:border-blue-500" /></div>
                  <div><p className="text-[8px] font-black text-red-500 uppercase mb-1">Negative Marking</p><input type="number" step="0.01" defaultValue={item.negativeMark || 0} onBlur={(e) => updateField(item.id, type, 'negativeMark', e.target.value)} className="w-full p-2.5 rounded-xl border border-white/10 bg-black text-white text-xs font-black outline-none" /></div>
                </div>
                <div className="flex flex-wrap gap-3">
                  <div className="bg-black p-2.5 rounded-xl border border-white/10 min-w-[120px]"><p className="text-[8px] font-black text-blue-400 uppercase mb-1">Time Limit</p><div className="flex items-center gap-1"><input type="number" defaultValue={item.hours} onBlur={(e) => updateField(item.id, type, 'hours', e.target.value)} className="w-10 text-center font-black bg-slate-900 rounded-lg outline-none text-white" /> <span className="font-bold text-[9px]">H</span><input type="number" defaultValue={item.minutes} onBlur={(e) => updateField(item.id, type, 'minutes', e.target.value)} className="w-10 text-center font-black bg-slate-900 rounded-lg outline-none text-white" /> <span className="font-bold text-[9px]">M</span></div></div>
                  <div className="flex-1 bg-black p-2.5 rounded-xl border border-white/10 shadow-sm"><p className="text-[8px] font-black text-slate-500 uppercase mb-1">Google Drive Link</p><input type="text" defaultValue={item.fileUrl} onBlur={(e) => updateField(item.id, type, 'fileUrl', e.target.value)} className="w-full p-2 rounded-lg border border-white/5 bg-black text-white text-[10px] outline-none font-bold" /></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><p className="text-[8px] font-black text-yellow-500 uppercase mb-1 italic">Answer Key</p><input type="text" defaultValue={item.answerKey} onBlur={(e) => updateField(item.id, type, 'answerKey', e.target.value.toUpperCase())} className="w-full p-2.5 bg-black border border-white/10 rounded-xl text-xs font-bold text-white outline-none" placeholder="A,B,W,D" /></div>
                  <div><p className="text-[8px] font-black text-green-500 uppercase mb-1 italic">Marks per Q</p><input type="text" defaultValue={item.questionMarks} onBlur={(e) => updateField(item.id, type, 'questionMarks', e.target.value)} className="w-full p-2.5 bg-black border border-white/10 rounded-xl text-xs font-bold text-white outline-none" placeholder="1,1,5,1" /></div>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="bg-slate-950/80 backdrop-blur-xl p-6 rounded-[2.5rem] shadow-2xl border-t-8 border-blue-700 w-full mb-8 text-left border-x border-b border-white/5">
        <div className="flex justify-between items-center mb-6"> <h3 className="font-black text-[10px] uppercase italic text-blue-400"><Zap size={20} className="inline mr-2"/> QUICK ADD HUB</h3>
          <div className="flex gap-1 p-1 bg-black rounded-xl border border-white/5"> 
            <button onClick={() => setQuickAddType('live')} className={`px-4 py-1.5 rounded-lg font-black text-[8px] transition-all ${quickAddType === 'live' ? 'bg-red-600 text-white' : 'text-slate-500'}`}>Live</button> 
            <button onClick={() => setQuickAddType('practice')} className={`px-4 py-1.5 rounded-lg font-black text-[8px] transition-all ${quickAddType === 'practice' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>Practice</button> 
          </div> 
        </div>
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
            <div><p className="text-[8px] font-black text-blue-400 uppercase mb-1">Class</p><select value={qaClass} onChange={(e) => setQaClass(e.target.value)} className="w-full p-2.5 bg-black border border-white/10 rounded-xl text-white text-[10px] font-black outline-none">{[5,6,7,8,9,10,11,12].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            <div><p className="text-[8px] font-black text-yellow-400 uppercase mb-1">Level</p><select value={qaLevel} onChange={(e) => setQaLevel(e.target.value)} className="w-full p-2.5 bg-black border border-white/10 rounded-xl text-white text-[10px] font-black outline-none"><option value="EASY">EASY</option><option value="MODERATE">MODERATE</option><option value="HARD">HARD</option></select></div>
            <div className="flex items-center gap-2 mt-4"><input type="checkbox" checked={qaGuest} onChange={(e) => setQaGuest(e.target.checked)} className="accent-blue-500 w-4 h-4" /><p className="text-[9px] font-black text-slate-400 uppercase italic">Enable Guest Access</p></div>
          </div>
          <div><p className="text-[8px] font-black text-slate-500 uppercase mb-1 italic">Exam Name</p><input type="text" value={qaName} onChange={(e) => setQaName(e.target.value)} className="w-full p-3.5 bg-black border border-white/10 rounded-2xl text-[10px] font-black outline-none focus:border-blue-500 text-white transition-all uppercase" placeholder="Module Name" /></div>
          <button onClick={handleQuickAdd} className="w-full bg-blue-700 text-white py-4 rounded-[1.5rem] font-black text-[11px] uppercase shadow-2xl active:scale-95 transition-all border-b-4 border-blue-900 italic tracking-tighter">Deploy to Registry</button>
        </div>
      </div>
      <PaperManager title="Live Mock" items={liveMocks} type="live" color="text-red-500" />
      ...
      <div className="bg-black/60 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl border-t-8 border-slate-900 w-full mb-20 text-center border-x border-b border-white/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {students.map((std) => (
            <div key={std.id} className="relative group p-5 bg-white/5 border border-white/10 rounded-[2rem] flex flex-col items-center hover:bg-white/10 transition-all text-center"> <input type="text" defaultValue={std.name} onBlur={async (e) => { if(e.target.value !== std.name) await setDoc(doc(db, "students", std.id), { name: e.target.value.toUpperCase() }, { merge: true }); }} className="bg-transparent text-center text-md font-black uppercase italic tracking-tighter text-white outline-none rounded-lg px-2" /> <div className="mt-2 flex items-center gap-2 bg-blue-950 px-3 py-1 rounded-full border border-blue-900"> <Lock size={10} className="text-blue-400" /> <input type="text" defaultValue={std.studentCode} onBlur={async (e) => { if(e.target.value !== std.studentCode) await setDoc(doc(db, "students", std.id), { studentCode: e.target.value }, { merge: true }); }} className="bg-transparent text-[10px] font-black text-blue-400 uppercase tracking-widest outline-none w-20 text-center" /> </div> <div className="flex gap-3 mt-6"> <button onClick={() => setSelectedStudent(std)} className="px-5 py-1.5 bg-slate-800 border border-slate-700 rounded-full text-[10px] font-black uppercase hover:bg-blue-600 transition-all italic">Reports</button> <button onClick={async (e) => { if (window.confirm(`Delete ${std.name}?`)) await deleteDoc(doc(db, "students", std.id)); }} className="p-2 bg-red-950/40 text-red-500 rounded-full border border-red-900/50 active:scale-90"><Trash2 size={16} /></button> </div> </div> ))} <button onClick={async () => { const n = prompt("Student Name:"); const c = prompt("Unique Code:"); if(n) await addDoc(collection(db, "students"), { name: n.toUpperCase(), studentCode: c || "" }); }} className="p-8 border-4 border-dashed border-white/10 rounded-[2.5rem] text-[12px] font-black text-slate-600 uppercase hover:text-blue-500 transition-all">+ REGISTER</button>
        </div>
      </div>
      ...
      <div className="bg-slate-900/60 backdrop-blur-md p-10 rounded-[3rem] border border-white/10 shadow-3xl max-w-xl mx-auto space-y-10">
        ...
        <div className="space-y-8 pt-8 border-t border-white/10 text-left">
          {results.filter(r => r.name === student?.name).sort((a, b) => b.timestamp - a.timestamp).map(r => (
            <div key={r.id} className="p-6 bg-white/5 border border-white/10 rounded-[2.5rem] flex flex-col gap-6 shadow-sm group">
              <div className="flex justify-between items-start w-full"><div className="flex items-center gap-4"><div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg bg-blue-900/40 text-blue-400 border border-blue-800/50 shadow-sm">{r.percent}%</div><div className="flex-1 min-w-0 pr-2"><p className="text-sm font-black uppercase italic tracking-tighter text-white leading-none break-words">{r.exam}</p><p className="text-[10px] font-bold text-slate-500 mt-1 italic">{r.date} • Score: {r.obtained}/{r.total}</p></div></div><button onClick={async () => { if (window.confirm("Purge record?")) await deleteDoc(doc(db, "results", r.id)); }} className="text-slate-600 hover:text-red-500 flex-shrink-0 print:hidden"><Trash2 size={24} /></button></div>
              {r.details && r.details.some(d => d.pending) && (
                <div className="bg-orange-950/30 border border-orange-900/50 rounded-[2rem] p-4 flex flex-col gap-3 shadow-inner print:hidden"><p className="text-[10px] font-black text-orange-400 uppercase italic text-center animate-pulse">Written Pending</p><div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar snap-x snap-mandatory text-left">
                  {r.details.filter(d => d.pending).map((pendingQ, pIdx) => {
                    ...
                    return photoList.map((photoUrl, imgIdx) => (
                      <div key={`${pIdx}-${imgIdx}`} className="min-w-[200px] bg-black border border-white/10 shadow-md rounded-2xl p-4 flex flex-col items-center gap-3 snap-center"><p className="text-[9px] font-black text-slate-500 uppercase italic">Q{pendingQ.qNum} - Page {imgIdx + 1}</p><button onClick={() => setPreviewImg(photoUrl)} className="w-full py-2 bg-blue-600 text-white rounded-xl font-black text-[9px] shadow-sm">View Page</button>
                        {imgIdx === photoList.length - 1 && (<div className="flex gap-2 w-full mt-2"><input id={`mark-input-${r.id}-${pendingQ.qNum}`} type="number" placeholder="Marks" className="w-1/2 p-2 border border-slate-700 rounded-xl text-center font-black text-[10px] focus:border-orange-500 bg-black text-white" /><button onClick={async () => {
                        ...
                        }} className="w-1/2 py-2 bg-orange-600 text-white rounded-xl font-black text-[9px] shadow-sm">Save</button></div>)}</div>));
...
const InteractiveExamHall = ({ exam, onFinish, studentsList }) => {
  ...
  const handleImageUpload = (qNum, file) => {
    if (exam.isGuest) return alert("Only MCQ for guests.");
    if (!file) return;
  ...
  if (isSubmitted) return (
    <div className="fixed inset-0 bg-slate-950 z-[2000] flex flex-col items-center overflow-y-auto p-10 text-center animate-in zoom-in duration-500 text-white"><CheckCircle size={80} className="text-green-500 mb-6 animate-bounce" /><h2 className="text-3xl font-black uppercase italic mb-8">Session Completed</h2><div className="bg-slate-900 p-10 rounded-[3rem] border-4 border-slate-800 mb-10 w-full max-sm shadow-2xl text-center"><h3 className="text-5xl font-black text-blue-400 italic leading-none">{scoreData?.correct} / {scoreData?.total}</h3></div><button onClick={onFinish} className="bg-blue-700 text-white px-16 py-4 rounded-full font-black uppercase shadow-2xl">Close Arena</button></div>
  );
  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col overflow-hidden animate-in fade-in duration-500">
      <div className="bg-slate-900 p-3 flex justify-between items-center border-b-4 border-yellow-500 shadow-2xl text-white"><div className="flex-1 min-w-0 pr-2"><h2 className="font-black text-[10px] uppercase italic truncate">{exam?.name}</h2><p className="text-[8px] text-blue-400 font-black uppercase mt-1 italic tracking-widest leading-none">{exam?.studentName} {exam.isGuest && '(GUEST)'}</p></div><div className="flex items-center gap-6"><div className="px-5 py-1.5 rounded-xl font-black text-2xl border-4 text-white border-slate-800 bg-black">{formatTime(timeLeft)}</div><button onClick={() => { if (window.confirm("SUBMIT?")) submitExam(); }} className="bg-green-600 text-white px-6 py-2 rounded-full font-black text-[10px] uppercase shadow-lg">SUBMIT</button></div></div>
      <div className="flex-1 bg-slate-950 overflow-hidden relative"><iframe src={exam?.fileUrl?.replace('/view?usp=sharing', '/preview').replace('/view', '/preview')} className="w-full h-full border-none opacity-90" title="Paper" /><div className="absolute bottom-0 left-0 right-0 z-50 bg-slate-900/98 border-t-2 border-white/10 backdrop-blur-xl p-4 shadow-2xl"><div className="max-w-4xl mx-auto"><div className="flex items-center justify-between mb-2 px-2 text-left"><span className="text-[9px] font-black text-blue-400 uppercase italic flex items-center gap-3"><PenTool size={16} /> RESPONSE INTERFACE</span>{activeQuestion && <button onClick={() => setActiveQuestion(null)} className="text-white bg-slate-700 px-3 py-1 rounded-lg font-black text-[10px] uppercase shadow-lg">Close</button>}</div>
        {activeQuestion ? (
          <div className="flex flex-col items-center animate-in slide-in-from-bottom-2 pb-2"><p className="text-slate-400 font-black text-xs mb-4 italic uppercase">{answerKeyArray[activeQuestion - 1] === 'W' ? `Capturing Q${activeQuestion}:` : `Choice Q${activeQuestion}:`}</p>
            {answerKeyArray[activeQuestion - 1] === 'W' ? (
              <div className="flex flex-col items-center gap-4">{exam.isGuest ? (<div className="p-4 bg-orange-900/20 border-2 border-orange-800 rounded-2xl text-center"><AlertCircle className="text-orange-500 mx-auto mb-2" /><p className="text-[9px] font-black text-orange-200 uppercase italic">MCQ ONLY FOR GUESTS.</p></div>) : (<>
                <div className="flex gap-2 flex-wrap justify-center text-left">{Array.isArray(answers[activeQuestion]) && answers[activeQuestion].map((_, i) => (<div key={i} className="relative"><div className="bg-green-600 text-white text-[8px] font-black px-2 py-1 rounded-lg">Page {i + 1} ✓</div><button onClick={() => removeImage(activeQuestion, i)} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-0.5 shadow-lg"><X size={12} /></button></div>))}</div>
...
const GrowthSectionView = ({ results, students }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full animate-in fade-in duration-500 text-left px-2">
      ...
      {!sel ? (
        <div className="grid gap-4 print:hidden text-left">{students.map((std) => (<button key={std.id} onClick={() => { setSel(std.name); setIsVerified(false); setVCode(''); }} className="w-full bg-black/60 backdrop-blur-xl p-5 rounded-[2rem] shadow-lg border border-white/10 flex justify-between items-center group active:scale-95 transition-all text-left"><div className="flex items-center gap-4 text-left"><div className="w-10 h-10 bg-blue-900/30 rounded-xl flex items-center justify-center text-blue-400 shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-all"><User size={18} /></div> <span className="font-black text-white uppercase text-[14px] italic tracking-tight break-words">{std.name}</span></div><ChevronRight size={24} className="text-slate-600 group-hover:text-blue-400" /></button>))}</div>
      ) : !isVerified ? (
        ...
        <div className="space-y-6 animate-in slide-in-from-right-20 duration-700 print-full-report text-left">
          ...
          <div className="bg-black/90 rounded-[3rem] shadow-2xl overflow-hidden border-2 border-white/10 flex flex-col print-full-report">
            <div className="bg-blue-700 p-8 text-white text-center relative overflow-hidden flex-shrink-0 print:border-b-4 print:border-blue-900"><Trophy className="absolute -top-10 -right-10 opacity-10 rotate-12 print:hidden" size={150} /><h2 className="text-2xl font-black uppercase italic tracking-tighter mb-2 px-4 text-white leading-none">Performance Transcript</h2><div className="inline-block bg-white/20 px-6 py-1.5 rounded-full border border-white/30 max-w-[90%] overflow-hidden"><p className="text-sm font-black uppercase italic break-words text-white">{sel}</p></div></div>
            <div className="p-4 md:p-6 space-y-4 bg-white/5 print:bg-white h-auto text-left">
              {(() => {
                ...
                return resultsWithAttempts.reverse().map((r) => {
                  ...
                  return (
                    <div key={r.id} className="w-full bg-slate-900/60 rounded-[2rem] border border-white/10 shadow-sm flex items-center p-4 md:p-5 gap-3 md:gap-6 hover:shadow-md transition-all group print-card text-left">
                      <div className="flex-1 min-w-0 border-l-4 md:border-l-8 border-blue-600 pl-3 md:pl-5 text-left">
                        ...
                        {hasPendingWritten && <p className="text-[7px] font-black text-orange-400 uppercase italic mt-0.5 animate-pulse">Review Pending</p>}
                        <p className="text-[8px] md:text-[9px] font-black text-blue-400 uppercase italic mt-1">{new Date(r.timestamp).toLocaleDateString('en-GB')} • ATTEMPT {r.attemptNo}</p></div>
                      <div className="text-center px-2 md:px-4 border-l border-white/10 min-w-[70px] md:min-w-[100px]"><p className="text-xl md:text-3xl font-black italic text-blue-400 leading-none">{r.obtained}/{r.total}</p></div>
                      <div className="flex-shrink-0 print:hidden"><button onClick={() => setSelectedReview(r)} className="bg-slate-800 text-blue-400 p-2 md:p-3 rounded-2xl border border-white/10 shadow-sm hover:bg-blue-600 transition-all"><Eye size={18}/></button></div>
                    </div> ); });
              })()}
            </div>
          </div>
