...
<p className="text-[14px] font-black text-white uppercase mt-5 italic animate-pulse tracking-tighter">Registered students, please wait for teacher's evaluation of written answers.</p>
        ...
        <div className="mt-8 space-y-2 max-h-60 overflow-y-auto no-scrollbar border-t border-slate-800 pt-4 w-full px-2">
          ...
          <div className="grid grid-cols-5 gap-2">
            {scoreData?.details?.map((item, idx) => (
              <div key={idx} className={`p-2 rounded-lg border flex flex-col items-center justify-between min-h-[55px] ${item.type === 'written' ? 'bg-orange-900/20 border-orange-800 text-orange-400' : (item.status ? 'bg-green-900/20 border-green-800 text-green-400' : 'bg-red-900/20 border-red-800 text-red-400')}`}>
  ...
  {/* এখানে আমরা টোটাল মার্কস দেখানোর লজিক যোগ করলাম */}
  <span className="text-[8px] font-bold mt-0.5 block italic bg-black/40 px-1 rounded">
    {item.type === 'written' ? `${item.mark}/${item.fullMark || 0}` : (item.status ? `${item.mark}/${item.mark}` : `0/${item.mark || 1}`)}
  </span>
</div>
            ))}
          </div>
