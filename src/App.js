...
const auth = getAuth(app); // Auth ইনিশিয়ালাইজ
...
const App = () => {
    ...
    const [user, setUser] = useState(null); // নতুন লগইন ইউজার স্টেট
    ...
    useEffect(() => {
        ...
        const unsubMocks = onSnapshot(collection(db, "liveMocks"), (s) => {
            const data = s.docs.map(d => ({ id: d.id, source: 'live', ...d.data() }));
            setLiveMocks(data.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)));
        });
        const unsubPractice = onSnapshot(collection(db, "practiceSets"), (s) => {
            const data = s.docs.map(d => ({ id: d.id, source: 'practice', ...d.data() }));
            setPracticeSets(data.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0)));
        });
        const unsubResults = onSnapshot(collection(db, "results"), (s) => setStudentResults(s.docs.map(d => ({ id: d.id, ...d.data() }))));
        const unsubStudents = onSnapshot(collection(db, "students"), (s) => setStudents(s.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => a.name.localeCompare(b.name))));
        const unsubSettings = onSnapshot(doc(db, "settings", "adminConfig"), (d) => {
        ...
        return () => {
            unsubscribeAuth(); unsubMocks(); unsubPractice(); unsubResults(); unsubStudents(); unsubSettings(); unsubLogs();
        };
    }, []);
    ...
    return (
        <div className="min-h-screen font-sans text-white select-none flex flex-col items-center overflow-x-hidden transition-all duration-700 bg-black" style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8)), url(${APP_BACKGROUND_URL})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} >
            ...
            <main className="w-full max-w-5xl p-4 mb-20 flex flex-col items-center">
                ...
                {/* --- 🛡️ SECURE ADMIN SECTION --- */}
                {activeTab === 'teacher' && (
                    !user ? (
                        <div className="max-w-md w-full mx-auto mt-20 p-10 bg-slate-950 rounded-3xl shadow-2xl text-center border-t-8 border-blue-700">
                            <User size={40} className="text-blue-500 mx-auto mb-6" />
                            <h3 className="text-white font-black uppercase mb-4 italic">Admin Verification</h3>
                        ...
                        <div className="max-w-md w-full mx-auto mt-20 p-10 bg-red-950 rounded-3xl text-center border-t-8 border-red-700 text-white">
                            ...
                            <button onClick={() => signOut(auth)} className="mt-6 text-[10px] font-bold underline uppercase">Logout & Switch Account</button>
                        </div>
...
// --- REST OF THE COMPONENTS (UNTOUCHED) ---
...
const GrowthSectionView = ({ results, students }) => {
    ...
    return (
        <div className="max-w-2xl mx-auto w-full animate-in fade-in duration-500 text-left px-2"> {selectedReview && <ReviewResultModal result={selectedReview} onClose={() => setSelectedReview(null)} />} {!sel ? (
            ...
            <div className="space-y-6 animate-in slide-in-from-right-20 duration-700 print-full-report"> <div className="flex justify-between items-center print:hidden"><button onClick={() => setSel(null)} className="flex items-center gap-2 text-[12px] font-black text-blue-400 uppercase italic hover:underline ml-2"><ChevronLeft size={24} /> Return</button><button onClick={handlePrint} className="bg-white text-black px-5 py-2 rounded-full font-black text-[10px] uppercase flex items-center gap-2 shadow-xl"><Download size={16} /> PDF</button></div> <div className="bg-black/90 rounded-[3rem] shadow-2xl overflow-hidden border-2 border-white/10 flex flex-col print-full-report"> <div className="bg-blue-700 p-8 text-white text-center relative overflow-hidden flex-shrink-0 print:border-b-4 print:border-blue-900"><Trophy className="absolute -top-10 -right-10 opacity-10 rotate-12 print:hidden" size={150} /><h2 className="text-2xl font-black uppercase italic tracking-tighter mb-2 leading-none break-words px-4 text-white">Performance Transcript</h2><div className="inline-block bg-white/20 px-6 py-1.5 rounded-full border border-white/30 max-w-[90%] overflow-hidden"><p className="text-sm font-black uppercase italic break-words text-white">{sel}</p></div></div> <div className="p-4 md:p-6 space-y-4 bg-white/5 print:bg-white print:overflow-visible h-auto"> {(() => { const studentResults = results.filter(r => r.name === sel).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0)); const examAttemptsMap = {}; const resultsWithAttempts = studentResults.map(r => { examAttemptsMap[r.exam] = (examAttemptsMap[r.exam] || 0) + 1; return { ...r, attemptNo: examAttemptsMap[r.exam] }; }); return resultsWithAttempts.reverse().map((r) => { const isMultiple = studentResults.filter(sr => sr.exam === r.exam).length > 1; const hasPendingWritten = r.details && r.details.some(d => d.type === 'written' && d.pending === true); return ( <div key={r.id} className="w-full bg-slate-900/60 rounded-[2rem] border border-white/10 shadow-sm flex items-center p-4 md:p-5 gap-3 md:gap-6 hover:shadow-md transition-all group print-card"> <div className="flex-1 min-w-0 border-l-4 md:border-l-8 border-blue-600 pl-3 md:pl-5"><p className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Exam Unit {isMultiple && <span className="text-yellow-500 ml-2">| ATTEMPT {r.attemptNo}</span>}</p><p className="text-xs md:text-lg font-black uppercase italic text-white leading-tight whitespace-normal break-words">{r.exam}</p> {hasPendingWritten && <p className="text-[7px] md:text-[8px] font-black text-orange-400 uppercase italic mt-0.5 animate-pulse">Score may increase after Anshu Sir's review</p>} <p className="text-[8px] md:text-[9px] font-black text-blue-400 uppercase italic mt-1">{new Date(r.timestamp).toLocaleDateString('en-GB')} • {new Date(r.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div> <div className="text-center px-2 md:px-4 border-l border-white/10 min-w-[70px] md:min-w-[100px]"><p className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase mb-0.5">Score</p><p className="text-xl md:text-3xl font-black italic text-blue-400 leading-none">{r.obtained}/{r.total}</p></div> <div className="flex-shrink-0 print:hidden"><button onClick={() => setSelectedReview(r)} className="bg-slate-800 text-blue-400 p-2 md:p-3 rounded-2xl border border-white/10 shadow-sm hover:bg-blue-600 hover:text-white transition-all"><Eye size={18} /></button></div> </div> ); }); })()} </div> </div> </div> )} </div> ); };
